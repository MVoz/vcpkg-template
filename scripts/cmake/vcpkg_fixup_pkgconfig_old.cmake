...
function(vcpkg_fixup_pkgconfig_check_libraries _config _contents_var _system_libs _system_packages _ignore_flags)
    ...
    ## Extra libraries:
    string(REGEX REPLACE "(^|;)/([a-zA-Z])/" "\\1\\2:/" _libs "${_contents}")
    ...
    string(REGEX REPLACE " /([a-zA-Z])/" ";\\1:/" _libs_list_tmp "${_libs}")
    string(REGEX REPLACE "-l/([a-zA-Z])/" "-l\\1:/" _libs_tmp "${_libs_list_tmp}")

    string(REGEX REPLACE "(^|;)/([a-zA-Z])/" "\\1\\2:/" _libs_private "${_contents}")
    string(REGEX REPLACE " /([a-zA-Z])/" ";\\1:/" _libs_private_list_tmp "${_libs_private}")
    string(REGEX REPLACE "-l/([a-zA-Z])/" "-l\\1:/" _libs_private_tmp "${_libs_private_list_tmp}")

    #message(STATUS "Found libraries: ${_libs_list_tmp}")
    #message(STATUS "Found private libraries: ${_libs_private_list_tmp}")
    list(APPEND _all_libs "${_libs_tmp}" "${_libs_private_tmp}")
