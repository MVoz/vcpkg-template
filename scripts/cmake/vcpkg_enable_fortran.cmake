...
#    foreach(INTEL_VERSION 18)
    ...
    foreach(CURRENT_PATH ${POTENTIAL_PATHS})
        ...
        if(EXISTS ${COMPILERVARS_BAT_PATH})
            ...
            list(APPEND CMAKE_PROGRAM_PATH "${DIRECTORY_NAME}/bin/${INTEL_ARCH}")
    ...
    if(CURRENT_COMPILERVARS_BAT_PATH)
        ...
        elseif("${VCPKG_PLATFORM_TOOLSET}" STREQUAL "v141")
            set(INTEL_VS "vs2017")
            # The Intel ifortvars.bat expects the environment variable VS2017INSTALLDIR to be present so we set it
            if(NOT "$ENV{VS2017INSTALLDIR}")
                set(ENV{VS2017INSTALLDIR} "$ENV{VSINSTALLDIR}")
            endif()
        elseif("${VCPKG_PLATFORM_TOOLSET}" STREQUAL "v142")
            set(INTEL_VS "vs2019")
            # The Intel ifortvars.bat expects the environment variable vs2019INSTALLDIR to be present so we set it
        ...
        foreach(POTENTIAL_PATH ${POTENTIAL_PATHS})
            if(IS_DIRECTORY ${POTENTIAL_PATH})
                ...
                if(EXISTS ${PGI_ENV_BAT_PATH})
                    ...
                    list(APPEND CMAKE_PROGRAM_PATH "${POTENTIAL_PATH}/bin")
                    if("${CURRENT_VERSION}" STREQUAL "" OR "${VERSION}" VERSION_GREATER "${CURRENT_VERSION}")
                        set(CURRENT_VERSION ${VERSION})
...
#    endif()
  ...
  set(ENV{PATH} "$ENV{PATH}${ENVIRONMENT_SEPARATOR}${MINGW_BIN_PATH}${ENVIRONMENT_SEPARATOR}${GFORTRAN_BIN}")
...
function(vcpkg_enable_fortran)
    if(DEFINED VCPKG_FORTRAN_COMPILER)
        ...
        message(STATUS "
    No Fortran compiler set in triplet, attempting to automatically set one.
    Please set VCPKG_FORTRAN_COMPILER in your triplet file.
    ")
